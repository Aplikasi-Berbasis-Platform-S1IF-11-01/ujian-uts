    ...
    <div class="admin-card">
        ...
        <div style="text-align:center;padding:12px 0">
            <div id="photo-preview-wrap" style="margin-bottom:20px">
                ...
                <img id="photo-preview" src="{{ asset('storage/'.$profile->photo) }}" style="width:120px;height:120px;border-radius:20px;object-fit:cover;border:2px solid var(--border)">
                @else
                <div id="photo-placeholder" style="width:120px;height:120px;border-radius:20px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;margin:0 auto;font-size:40px;color:#fff;font-weight:700">
                    {{ $profile ? strtoupper(substr($profile->name,0,1)) : 'A' }}
                </div>
                @endif
            </div>
        ...
        <form id="form-profile">
            <div style="display:flex;flex-direction:column;gap:14px">
                <div>
                    ...
                    <input class="form-ctrl" type="text" name="name" value="{{ $profile->name ?? '' }}" required>
                </div>
                <div>
                    <label class="form-label-admin">Judul / Title</label>
                    <input class="form-ctrl" type="text" name="title" value="{{ $profile->title ?? '' }}" placeholder="UI Developer">
                </div>
                <div>
                    <label class="form-label-admin">NIM</label>
                    <input class="form-ctrl" type="text" name="nim" value="{{ $profile->nim ?? '' }}" placeholder="2311102167">
                </div>
                <div>
                    <label class="form-label-admin">Universitas</label>
                    <input class="form-ctrl" type="text" name="university" value="{{ $profile->university ?? '' }}">
                </div>
                <div>
                    <label class="form-label-admin">GitHub Username</label>
                    <input class="form-ctrl" type="text" name="github_username" value="{{ $profile->github_username ?? '' }}" placeholder="andikaneviantoro">
                </div>
                <div>
                    <label class="form-label-admin">Status Label</label>
                    <input class="form-ctrl" type="text" name="status_label" value="{{ $profile->status_label ?? '' }}" placeholder="Available for work">
                </div>
                <div>
                    <label class="form-label-admin">Deskripsi</label>
                    <textarea class="form-ctrl" name="description" rows="4" style="resize:vertical">{{ $profile->description ?? '' }}</textarea>
                </div>
                <div>
...
btnUpload.addEventListener('click', async function() {
    ...
    this.disabled = true;
    try {
        const res = await ajaxRequest('{{ route("admin.profile.photo") }}', 'POST', formData);
        if (res.success) {
...
document.getElementById('form-profile').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await ajaxRequest('{{ route("admin.profile.update") }}', 'POST', data);
